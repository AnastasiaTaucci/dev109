...
function isValid() {
  ...
  if (
    ...
    document.getElementById("submiterror").innerHTML =
      "<p>Error Submitting — See Above</p>";
    return false;
  }
}
...
function tipPassword() {
  ...
  elPasswordMsg.innerHTML =
    "Maximum 7 characters. Must include 1 upper, 1 lower, 1 number and 1 special character.";
}
...
function checkFirstName() {
  ...
  if (firstname === null || firstname === "") {
    ...
    elFirstNameMsg.className = "warning"; // Change class on message
    elFirstNameMsg.textContent =
      "The first name cannot be greater than 20 characters";
    console.log("First name invalid — length");
  } else if (firstname.match("^[a-zA-Z ,.'-]+$") === null) {
    elFirstNameMsg.className = "warning"; // Change class on message
    elFirstNameMsg.textContent =
      "Invalid caracter in first name (accepts only A-Z, a-z, and ,.'-)";
...
function checkLastName() {
  ...
  if (lastname === null || lastname === "") {
    ...
    elLastNameMsg.className = "warning"; // Change class on message
    elLastNameMsg.textContent =
      "The last name cannot be greater than 50 characters";
    console.log("Last name invalid — length");
  } else if (lastname.match("^[a-zA-Z ,.'-]+$") === null) {
    elLastNameMsg.className = "warning"; // Change class on message
    elLastNameMsg.textContent =
      "Invalid caracter in last name (accepts only A-Z, a-z, and ,.'-)";
...
function checkEmail() {
  ...
  var userEmail = elEmail.value.trim();
...
function checkPhone() {
  ...
  if (isNaN(phone) || phone.length > 15) {
    ...
    if (phone.length > 6) {
      ...
      console.log("two - will be added " + elPhone.value);
    }
...
function checkZipCode() {
  ...
  var country = elCountry.value.trim();
  var zipcode = elZipCode.value.trim();
  if (country === "USA") {
